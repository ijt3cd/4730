...
public class Tween {
	...
	public void animate(TweenParam param) {
		...
		double value = (double) ((float) (System.currentTimeMillis() - param.getStartTime())
				/ (float) param.getTweenTime());
		if (transition != null) {
	...
	public void setValue(TweenableParams param, double value) {
		...
		case Y:
			...
			break;
		case SCALE_X:
			tweenee.setScaleX(value);
			break;
		case SCALE_Y:
			tweenee.setScaleY(value);
			break;
		case VELOCITY_X:
			tweenee.setVelocityX((float) value);
			break;
		case VELOCITY_Y:
			tweenee.setVelocityY((float) value);
			break;
		case ALPHA:
			tweenee.setAlpha((float) value);
			break;
		case ROTATION:
			tweenee.setRotation(value);
			break;
		case VISIBLE:
			if (value == 1)
				tweenee.setVisible(true);
			if (value == 0)
				tweenee.setVisible(false);
		}
	}
}
